...
gradient_descent_backtracking <- function(tol = 1e-6, max_iter = 10000, epsilon = 0.5, tau = 0.8) {
  # Step 1: Initialize and select a random stopping point
  # Initialize
  set.seed(777) # example seeding 
  last_iter <- 0 # the last iteration ran
  ...
  armijo <- function(eta_k, x_k, epsilon) {
    h <- function(eta_k, x_k) {
      return(obj_function(x_k) - (eta_k * grad(x_k)))
    }
    h_prime <- "init"
  ...
  for (iter in 1:max_iter) { # the iteration goes n = 1, 2, 3, 4, but the arrays of our output starts at iter = 0 and guess x0
    ...
    if(iter > 1) {
      sub_iter = 1
      while(!armijo(eta) || sub_iter < 10000) {
        eta <- eta * tau
        sub_iter <- sub_iter + 1
      }
    }
